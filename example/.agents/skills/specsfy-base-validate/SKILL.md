...
anuncie `Transição automática: $specsfy-base-validate → $<destino> — motivo:
...
automática: $<destino> → $specsfy-base-validate — pendência resolvida:
...
6. Compare research, requisitos, BDD, plano técnico, modelo de dados, contratos, TDD, matriz e tarefas. Não confunda “arquivo bem formatado” com “especificação correta”.
...
Inclua contagens, cobertura `FR/NFR ↔ AC/verificação`, os três achados mais
importantes e a transição automática:

- `READY` → `$specsfy-base-tasks` para planejar a seção 14;
- `NOT READY` → retorno a `$specsfy-base-interview` quando faltar decisão ou a
  `$specsfy-base-specify` quando a correção já estiver decidida.
...
```bash
python3 -B .agents/skills/specsfy-base-validate/scripts/verify_repo.py . \
...
O contrato do catálogo exige as oito skills `specsfy-base-*`, `specsfy-setup`,
