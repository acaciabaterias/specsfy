...
anuncie `Transição automática: $specsfy-base-tdd-bdd → $<destino> — motivo:
...
automática: $<destino> → $specsfy-base-tdd-bdd — pendência resolvida:
...
- Em PHP, escreva o teste TDD com Pest em `tests/Feature/` ou `tests/Unit/`, no
  menor nível que prova o comportamento. Marque o teste com:
...
- Cubra caminho feliz, regra crítica e falha material, sem multiplicar cenários equivalentes.
...
`$specsfy-base-tasks`, que valida todos os predecessores antes de promover a
spec para `Planned`.
...
```bash
python3 .agents/skills/specsfy-base-tdd-bdd/scripts/check_traceability.py specs/specs/<NNNN>-<slug>/spec.md .
```

Trate IDs sem teste como gaps. Para `NFR` verificado manualmente ou por observabilidade, cite a evidência e, quando necessário, execute com `--kinds FR,AC,NFR`.
Em specs com `Evidence Contract: 1`, acrescente `--full-chain` para exigir
`FR/NFR/AC → teste → tarefa → evidência`.
...
```bash
python3 -B .agents/skills/specsfy-base-tdd-bdd/scripts/verify_acceptance.py \
