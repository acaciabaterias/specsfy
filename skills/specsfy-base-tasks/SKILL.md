...
5. Se faltar uma decisão que mude arquitetura, dados ou aceite, anuncie e
   retorne automaticamente para `$specsfy-base-interview`. Depois da decisão,
   use `$specsfy-base-update-spec` quando a spec já tiver sido aprovada ou
   `$specsfy-base-specify` durante a definição inicial.
