...
anuncie `Transição automática: $specsfy-base-progress → $<destino> — motivo:
...
automática: $<destino> → $specsfy-base-progress — pendência resolvida:
...
```bash
python3 -B .agents/skills/specsfy-base-progress/scripts/progress.py .
```

Para integração com outras ferramentas:

```bash
python3 -B .agents/skills/specsfy-base-progress/scripts/progress.py . --json
```

Para uma única especificação:

```bash
python3 -B .agents/skills/specsfy-base-progress/scripts/progress.py . --slug <slug>
```

Para observar o custo de contexto de uma spec:

```bash
python3 -B .agents/skills/specsfy-base-progress/scripts/analyze_context.py \
...
7. Trate `docs/` ausente ou desatualizado segundo `$specsfy-documentator` como
   blocker da entrega.
8. Se nenhuma spec existir, preserve o código 2, anuncie a pendência e carregue
