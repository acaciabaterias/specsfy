...
describe("renderização e interação em terminal real", () => {
  ...
  test("filtra e marca skills com bytes reais e distintos de Tab e Enter", async () => {
    ...
    await settle();
    expect(screenText(screen)).toContain("16 visível(is)");
  ...
  test("alterna, limpa e marca novamente a seleção por teclado", async () => {
    ...
    await settle();
    expect(screenText(screen)).toContain(
      "3 selecionada(s) · 16 visível(is) · 1 para instalar · 0 para remover",
    );
    press(screen, "enter");
    await settle();
    expect(screenText(screen)).toContain(
      "2 selecionada(s) · 16 visível(is) · 0 para instalar · 0 para remover",
    );
    press(screen, "l", true);
    await settle();
    expect(screenText(screen)).toContain(
      "0 selecionada(s) · 16 visível(is) · 0 para instalar · 2 para remover",
    );
    press(screen, "v", true);
    await settle();
    expect(screenText(screen)).toContain(
      "16 selecionada(s) · 16 visível(is) · 14 para instalar · 0 para remover",
    );
  });
