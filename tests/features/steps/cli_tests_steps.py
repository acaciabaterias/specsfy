...
TESTING = ROOT / "cli/src/specsfy_cli/testing.py"
...
def then_cli_detects_laravel_pest_safely(context) -> None:
    ...
    assert "create_subprocess_shell" not in context.testing
    assert "shell=True" not in context.testing
...
def then_cli_test_streams_and_preserves_exit_code(context) -> None:
    assert 'commands.add_parser("test"' in context.app
    assert "run_project_tests(args.project)" in context.app
    assert "return result.exit_code" in context.app


@then("a TUI separa o resumo e os testes em subabas")
def then_tui_separates_summary_and_tests(context) -> None:
    assert 'TabPane("Resumo", id="tab-tests-summary")' in context.tests_markup
    assert 'TabPane("Testes", id="tab-tests-output")' in context.tests_markup
    assert 'id="tests-summary"' in context.tests_markup


@then("a saída detalhada permanece rolável e a execução é explícita")
def then_tui_has_detailed_live_log(context) -> None:
    assert "RichLog(" in context.tests_markup
    assert 'id="tests-output"' in context.tests_markup
    assert '"Executar testes  ^X"' in context.tests_markup
    assert 'id="run-tests"' in context.tests_markup
    assert "stream_project_tests(" in context.tui
