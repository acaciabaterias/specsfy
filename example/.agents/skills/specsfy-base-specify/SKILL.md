...
description: "Use quando o usuário pede para promover um backlog entrevistado, criar, iniciar, consolidar ou atualizar uma especificação em `spec.md`, inclusive ao fornecer requisitos ou mudar comportamento. Use também quando uma transição automática exigir criar, corrigir ou reabrir a fonte normativa. Inicializa specs em `specs/specs/` e aplica o MCR-10; para ideia superficial use specsfy-base-backlog, para aprofundamento conversacional use specsfy-base-interview e para revisão sem edição use specsfy-base-validate."
---
...
anuncie `Transição automática: $specsfy-base-specify → $<destino> — motivo:
...
automática: $<destino> → $specsfy-base-specify — pendência resolvida:
...
3. Use o caminho absoluto impresso pelo script. Ele aloca o próximo ID local,
   copia o template gerenciado `.specsfy/templates/Spec.md` e cria somente
   `specs/specs/<NNNN>-<slug>/spec.md`; nunca renomeie o arquivo para incluir o ID.
   Ao desenvolver este repositório, o script usa `templates/Spec.md` como
...
5. Se existir lacuna P1, faça uma pergunta por vez na linguagem do usuário. Não
   escreva uma spec `Defined` enquanto ela permanecer.
...
- Preserve exatamente os três atos e as 18 seções de
...
- Mudança de comportamento, aceite, escopo, dados ou segurança reabre o Ato I:
  defina `Status: Draft`, `Definition Gate: Pending`, `Plan Gate: Pending` e
  `Delivery Gate: Pending`.
- Mudança apenas no plano, nas tarefas ou na estratégia de testes reabre o Ato II:
  defina `Status: Defined`, mantenha `Definition Gate: Passed` e retorne
  `Plan Gate` e `Delivery Gate` para `Pending`.
- Nunca mantenha um gate posterior aprovado depois que sua entrada foi invalidada.

## Preservar rastreabilidade

Para cada `FR` e `NFR`, aponte cenários `AC` ou um método de verificação explícito. Para cada história, identifique os requisitos que entregam seu valor. Use os mesmos IDs mais tarde em testes e na seção 14.

## Controlar research e mudanças

- Antes do planejamento, carregue somente as evidências indexadas e valide
  claims com:

```bash
python3 -B .agents/skills/specsfy-base-specify/scripts/load_research.py \
...
- Antes de alterar uma spec versionada, projete o impacto com
  `analyze_change.py specs/specs/<NNNN>-<slug>/spec.md --base HEAD --mode impact`. Use o modo
  `changelog` para listar IDs adicionados, removidos e alterados.
- Consuma `changed_areas` (`definition`, `plan`, `evidence` ou `unknown`) para
  decidir o handoff. A classificação usa títulos canônicos, não números de
  seção; `unknown` reabre do Ato I por segurança.
- Impacto e changelog são somente leitura. A pessoa autorizada decide a mudança;
  então aplique a invalidação de gates prevista nesta skill.
...
```bash
python3 .agents/skills/specsfy-base-validate/scripts/validate_spec.py specs/specs/<NNNN>-<slug>/spec.md --allow-draft
```

Corrija falhas estruturais em no máximo três ciclos. A skill `specsfy-base-validate` faz a revisão semântica, registra o resultado na seção 13 e promove `Definition Gate: Passed` e `Status: Defined`. Até lá, mantenha `Status: Draft`, `Definition Gate: Pending` e relate decisões bloqueantes.
...
- transição automática para `$specsfy-base-validate`, com motivo e resultado
  esperado.
