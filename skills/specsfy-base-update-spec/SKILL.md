...
anuncie `Transição automática: $specsfy-base-update-spec → $<destino> — motivo:
...
automática: $<destino> → $specsfy-base-update-spec — pendência resolvida:
...
5. Se não existir uma spec, anuncie a pendência e carregue
   `$specsfy-base-backlog` para uma ideia superficial ou
   `$specsfy-base-specify` quando já houver intenção explícita de criar a fatia.
6. Se a spec ainda estiver em `Draft` e nunca tiver obtido
   `Definition Gate: Passed`, carregue `$specsfy-base-specify`; esta skill
...
- Para capacidade independente da finalidade e dos atores da fatia atual, não a
  acrescente silenciosamente: carregue `$specsfy-base-backlog` ou
  `$specsfy-base-specify` conforme a maturidade do pedido.
- Quando opções plausíveis mudarem materialmente o resultado, faça handoff para
  `$specsfy-base-interview`, obtenha uma decisão por vez e retome esta skill.
...
```bash
python3 -B .agents/skills/specsfy-base-update-spec/scripts/analyze_change.py \
...
- Preserve tarefas e evidências ainda válidas; `$specsfy-base-tasks` reconcilia
  itens afetados e suas dependências antes de novo RED.

Valide a estrutura intermediária:

```bash
python3 -B .agents/skills/specsfy-base-validate/scripts/validate_spec.py \
  specs/specs/<NNNN>-<slug>/spec.md --allow-draft
```

## Retomar o fluxo

- Mudança desde o Ato I: carregue `$specsfy-base-validate`; depois percorra
  tasks, TDD/BDD e a etapa que originou a mudança.
- Mudança somente do Ato II: carregue `$specsfy-base-tasks`, que reconcilia o
  plano, chama `$specsfy-base-tdd-bdd` e retoma a origem.
- Mudança detectada durante implementação: retome
  `$specsfy-base-implement` somente depois de Definition e Plan Gate voltarem a
