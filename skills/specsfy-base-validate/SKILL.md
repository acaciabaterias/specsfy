...
- `NOT READY` → retorno a `$specsfy-base-interview` quando faltar decisão;
  quando a correção já estiver decidida, use `$specsfy-base-update-spec` para
  uma spec anteriormente aprovada e `$specsfy-base-specify` para a definição
  inicial.
...
O contrato do catálogo exige as nove skills `specsfy-base-*`, `specsfy-setup`,
